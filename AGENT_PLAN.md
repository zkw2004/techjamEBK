...
| B10-fix | **New — correction to B10, not new work.** `pcr_hist`'s duration confound. Measured: raw play-completion ratio is duration-confounded **5.2×** across the duration range, while `long_view` itself is only confounded **1.4×** — KuaiRand's own labelling already applies a duration-dependent threshold. `pcr_hist` aggregates the *more* confounded raw signal to help predict the *less* confounded label, which may be net-negative. See `RECSYS_COURSE_ANALYSIS.md` §2.4/§3.4. | B10 | Re-derive `pcr_hist` from a duration-normalised per-bucket ratio (compare against the bucket's own median completion, not raw play_time/duration) rather than the raw ratio. Ablate old vs new via A10 at screen tier; report the delta. A regression is an acceptable, reportable outcome. |
