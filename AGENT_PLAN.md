...
| A12 | Citations in hypotheses. Keep the `Action` schema unchanged: there is no `references` field. `propose()` carries a self-contained idea bank in code and requires the model to append one citation to `hypothesis` as `[ref: ...]`. Run-log rendering prints the hypothesis with that inline citation. | A1, A3 | `Action` still rejects a `references` field. `agent/prompts/idea_bank.md` is not created. An uncited proposal is re-asked and may change only by adding the citation. Rendered run log shows `[ref: ...]` next to the hypothesis. |
...
| C10 | **New — gated on B14.** DIN: attention over the user's LastN history (n ≈ 10–30, median 31 rows/user measured) with a time-since-interaction embedding, replacing plain mean-pooling for the history representation used alongside `sim_to_history`. See §3.2. **Do not build if B14 lands inside noise** — SIM-style truncation is explicitly out (sequences are too short to need it; see §2.3). | B14, C5 | **NOT FUNDED — do not build.** B14's confirm-tier gate returned delta −0.000561 with a 95% CI [−0.001456, +0.001082] straddling zero, on a corrected FM encoder (trap 14). By B14's own decision rule, `sim_to_history` landing inside noise means C10 is not built. This is the cheap probe doing its job: it closed out the organisers' #2-ranked headroom direction for ~13 minutes of compute instead of DIN's build cost. Reopen only if a *different* history representation shows signal first — the gate tested one feature, not every possible encoding of user history. |
...
14. **A model that silently mis-encodes a feature makes every experiment on that feature meaningless.** FM's `_fit_encoder` was categorical-only: it built an exact-value vocabulary over *every* column, with no numeric path (DeepFM has always had one — `numeric_edges`, 32 quantile bins). So FM memorised continuous features value-by-value. Measured on the real split: `pcr_hist` produced **1,003,882 distinct values over 1.14M training rows with 100% of validation rows unseen** — the feature was a literal constant at scoring time, after polluting training with a million single-observation embeddings that the second-order term mixes into all five real fields. **Thirteen of ~20 registered features were affected, eight at 100% unseen.** Cost, measured on `sim_to_history`: −0.0052 primary with a 95% CI entirely below zero. After the fix the same experiment gives −0.00056 with a CI straddling zero — i.e. **the bug accounted for ~90% of what looked like a scientific result**. Two lessons. (a) *A confidently negative result is a bug signal.* A feature carrying no information lands near zero; one that actively costs 0.005 is usually being mishandled, not merely useless — treat a strongly negative delta as a reason to inspect the pipeline before believing it. (b) *Check what the model actually receives.* `pipeline.train._new_model` had been passing `feature_names` to every model since it was written; FM swallowed it into `**hparams` and ignored it, so nothing failed and nothing warned. The diagnostic that found this — distinct-value count per column, plus the unseen-at-validation fraction — costs seconds and should be run for any new model or feature type.
...
Mapped to the confirmed challenge brief, Section 2.5. Track 2 does not require
a video; an approximately three-minute video is recommended when it helps
explain the solution effectively. Without one, a detailed report is strongly
encouraged. The four required groups are the written Devpost description,
public repository, run/iteration logs, and final submission/results/resources.
...
Feeds A12's propose prompt. A12 is intentionally self-contained: the active idea bank lives in `agent/propose.py`, no `agent/prompts/idea_bank.md` file is created, and citations are embedded in `Action.hypothesis` as `[ref: ...]` rather than stored in a schema field.
